...
function theme_precmd {
    ...
    local venvprompt=`virtualenv_prompt_info || conda_prompt_info`
    local venvpromptsize=${#${venvprompt}}
    local rubyprompt=`rvm_prompt_info || rbenv_prompt_info`
    local rubypromptsize=${#${rubyprompt}}
    local pwdsize=${#${(%):-%~}}

    if [[ "$promptsize + $venvpromptsize + $rubypromptsize + $pwdsize" -gt $TERMWIDTH ]]; then
      ((PR_PWDLEN=$TERMWIDTH - $promptsize))
    else
      PR_FILLBAR="\${(l.(($TERMWIDTH - ($promptsize + $venvpromptsize + $rubypromptsize + $pwdsize)))..${PR_HBAR}.)}"
    fi
...
$PR_CYAN$PR_ULCORNER$PR_HBAR`virtualenv_prompt_info``conda_prompt_info`$PR_GREY(\
$PR_GREEN%$PR_PWDLEN<...<%~%<<\
